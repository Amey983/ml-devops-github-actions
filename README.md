...
## SARIMA Diwali Electronics Forecast

This project also includes `sarima_diwali_forecast.py`, which builds a SARIMA demand forecast for three electronics items during the Diwali 2019 season.

Dataset used: public GitHub electronics-store sales dataset from `sinjoysaha/sales-analysis`, containing 2019 order-level transactions with `Order Date`, `Product`, `Quantity Ordered`, `Price Each`, and `Purchase Address`.

Default forecast items:
...
- Aggregate transactions into daily item-level demand.
- Train on data before the Diwali window.
- Forecast `2019-10-20` through `2019-11-03`, centered on Diwali `2019-10-27`.
- Use `SARIMA(1,1,1)(1,0,1,7)` to capture trend plus weekly seasonality.

Run the model:

```bash
python sarima_diwali_forecast.py
```

Generated files:

- `outputs/diwali_sarima_forecast.csv`
- `outputs/sarima_diagnostics.csv`
